<?php

namespace Database\Seeders;

use Illuminate\Database\Console\Seeds\WithoutModelEvents;
use Illuminate\Database\Seeder;
use App\Models\Folder;
use App\Models\File;
use App\Models\User;

class FileFolderSeeder extends Seeder
{
    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        $folderDatas = [
            [
                "name" => "Uploads",
                "parent" => null
            ],
            [
                "name" => "Images",
                "parent" => "Uploads"
            ],
            [
                "name" => "Document",
                "parent" => null
            ],
            [
                "name" => "Videos",
                "parent" => null
            ],
            [
                "name" => "Sources",
                "parent" => null
            ],
            [
                "name" => "Shares",
                "parent" => null
            ],
        ];
        $fileDatas = [
            [
                "name" => "Seasandiego.jpg",
                "url" => "https://assets.site-static.com/userFiles/2597/image/2023/CARDIFF_BY_THE_SEA/5-23-2023_1__reasons-why-cardiff-by-the-sea-san-diego-great-place-to-live/9_Reasons_Why_Cardiff-by-the-Sea_San_Diego_.jpg",
                "type" => "image/jpg",
                "dimension" => "2000x2000",
                "size" => "763300",
                "photo_by" => "Admin",
                "folder" => "Uploads"
            ],
             [
                "name" => "iMactwin.png",
                "url" => "https://i.insider.com/60e4cb0d22d19400191c957c?width=1000&format=jpeg&auto=webp",
                "type" => "image/jpg",
                "dimension" => "2000x2000",
                "size" => "640200",
                "photo_by" => "Apple",
                "folder" => "Uploads"
            ],
            [
                "name" => "hustlecup.jpg",
                "url" => "https://images.deliveryhero.io/image/fd-ph/LH/kmph-hero.jpg",
                "type" => "image/jpg",
                "dimension" => "2000x2000",
                "size" => "100400",
                "photo_by" => "Admin",
                "folder" => "Uploads"
            ],
            [
                "name" => "MS-Surface.jpg",
                "url" => "https://img-prod-cms-rt-microsoft-com.akamaized.net/cms/api/am/imageFileData/RE4OXzi?ver=3a58",
                "type" => "image/jpg",
                "dimension" => "2000x2000",
                "size" => "113200",
                "photo_by" => "Admin",
                "folder" => "Uploads"
            ],
            [
                "name" => "Famoustower.jpg",
                "url" => "https://www.worldfamousthings.com/wp-content/uploads/2018/01/Leaning-Tower-of-Pisa-Italy.jpg",
                "type" => "image/jpg",
                "dimension" => "2000x2000",
                "size" => "763300",
                "photo_by" => "Admin",
                "folder" => "Uploads"
            ],
            [
                "name" => "GODZILLA MINUS ONE Official Trailer",
                "url" => "https://upload.wikimedia.org/wikipedia/commons/thumb/6/65/No-Image-Placeholder.svg/330px-No-Image-Placeholder.svg.png?20200912122019",
                "type" => "video/mp4",
                "dimension" => "2000x2000",
                "size" => "763300",
                "photo_by" => "Admin",
                "folder" => "Videos"
            ],
            [
                "name" => "Marvel Studios’ Loki Season 2 | October 6 on Disney+",
                "url" => "https://upload.wikimedia.org/wikipedia/commons/thumb/6/65/No-Image-Placeholder.svg/330px-No-Image-Placeholder.svg.png?20200912122019",
                "type" => "image/jpg",
                "dimension" => "2000x2000",
                "size" => "763300",
                "photo_by" => "Admin",
                "folder" => "Videos"
            ],
            [
                "name" => "Marvel Studios’ Loki Season 2 | October 6 on Disney+",
                "url" => "https://upload.wikimedia.org/wikipedia/commons/thumb/6/65/No-Image-Placeholder.svg/330px-No-Image-Placeholder.svg.png?20200912122019",
                "type" => "image/jpg",
                "dimension" => "2000x2000",
                "size" => "763300",
                "photo_by" => "Admin",
                "folder" => "Videos"
            ],
            [
                "name" => "THE BOY AND THE HERON | Official Teaser Trailer",
                "url" => "https: //upload.wikimedia.org/wikipedia/commons/thumb/6/65/No-Image-Placeholder.svg/330px-No-Image-Placeholder.svg.png?20200912122019",
                "type" => "image/jpg",
                "dimension" => "2000x2000",
                "size" => "763300",
                "photo_by" => "Admin",
                "folder" => "Videos"
            ],
        ];

        foreach ($folderDatas as $folderData) {
            if ($folderData['parent'] !== null) {
                $parent = Folder::where('name', $folderData['parent'])->first();
                Folder::create([
                    'name' => $folderData['name'],
                    'parent_id' => $parent?->id,
                ]);
            } else {
                Folder::create(["name" => $folderData['name']]);
            }
        }


        foreach ($fileDatas as $fileData) {
            $folder = Folder::where('name', $fileData['folder'])->first();
            $user = User::where('name', $fileData['photo_by'])->first();

            File::create([
                'name' => $fileData['name'],
                'url' => $fileData['url'],
                'type' => $fileData['type'],
                'dimension' => $fileData['dimension'],
                'size' => $fileData['size'],
                'folder_id' => $folder?->id,
                'photo_by' => $user?->id,
            ]);
        }
    }
}
